...
def update_top_bargraphs(commodity, activity, county, operators, selection):
    ...
    # Get top 5 counties
    df_counties = (
        df[["PRODUCES", "COUNTY", "PRODUCTION"]]
        .groupby(["PRODUCES", "COUNTY"])
        .sum()["PRODUCTION"]
        .reset_index()
    ...
    # Get top 5 Operators
    df_operators = (
        df[["PRODUCES", "OPERATOR", "PRODUCTION"]]
        .groupby(["PRODUCES", "OPERATOR"])
        .sum()["PRODUCTION"]
        .reset_index()
        .sort_values(by="PRODUCTION", ascending=False)
        .groupby("PRODUCES")
    ...
    df_operators["TRUNCATED_NAME"] = df_operators["OPERATOR"].apply(
        lambda x: x[:6] + "..."
    )
    ...
    for i, commo in enumerate(commodity):
        ...
        # Add county traces
        traces_counties = px.bar(
            df_counties[df_counties["PRODUCES"] == commo.upper()],
            x="PRODUCES",
            y="PRODUCTION",
            text="COUNTY",
            color="COUNTY",
            ...
            barmode="overlay",
            color_discrete_sequence=clrscle,
        ).data
        ...
        # Add operator traces
        traces_operators = px.bar(
            df_operators[df_operators["PRODUCES"] == commo.upper()],
            x="PRODUCES",
            y="PRODUCTION",
            text="TRUNCATED_NAME",
            hover_name="OPERATOR",
        ...
        fig_counties.update_xaxes(
            showline=True, linewidth=2, linecolor="black", mirror=True, gridwidth=0.5
        )
